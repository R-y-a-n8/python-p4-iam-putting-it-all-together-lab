# Ensure this is imported from your actual app structure
from app import create_app  # Import the app creation function
from models import db, User, Recipe  # Import the necessary models
import pytest
...
class TestSignup:
    ...
    def test_creates_users_at_signup(self):
        ...
        # Ensure the DB is clean before starting tests
...
class TestRecipeIndex:
    ...
    def test_lists_recipes_with_200(self):
        ...
        with app.app_context():
            ...
            fake = Faker()
            user = User(username="Slagathor", bio=fake.paragraph(nb_sentences=3), image_url=fake.url())
            ...
            for i in range(15):
                ...
                recipe = Recipe(title=fake.sentence(), instructions=instructions, minutes_to_complete=randint(15, 90))
                recipe.user = user
    ...
    def test_get_route_returns_401_when_not_logged_in(self):
        '''returns a 401 Unauthorized status code if not logged in'''
        with app.app_context():
            Recipe.query.delete()
            User.query.delete()
            db.session.commit()
    ...
    def test_creates_recipes_with_201(self):
        '''creates a recipe and returns 201 status.'''
        with app.app_context():
            Recipe.query.delete()
            User.query.delete()
            db.session.commit()
    ...
    def test_returns_422_for_invalid_recipes(self):
        '''returns a 422 error for invalid recipe data'''
        with app.app_context():
            Recipe.query.delete()
            User.query.delete()
            db.session.commit()
